...
parser = argparse.ArgumentParser('LaTeX/Coq Detokenizer')
...
def check_proof(coq, tgt, subset=""):
    ...
    try:
        res = proc.communicate(coq + "\n", timeout=600)[0]
        valid_proof = ("No more goals." in res or "No more subgoals." in res)
    except:
        valid_proof = False
...
def validate(filename):
    ...
    for example in examples:
        ...
        if "even-odd" in seq[3]:
            ...
            if "e" in seq[3].split('/')[-1]:
                subset = "even-odd"
                detok_sub = "even-odd"
            elif "p" in seq[3].split('/')[-1]:
                subset = "powers"
                detok_sub = "powers"
            elif "c" in seq[3].split('/')[-1]:
                subset = "composites"
                detok_sub = "composites"
            else:
                detok_sub = "misc"
