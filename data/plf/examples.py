...
from grammars import EvenOdd, Powers, Composites, Straightline
...
def even_odd_old(count):
    ...
    seed = np.array([
        # Meta choices.
        np.random.choice(num_options[args.set]),            #  0: Number of nats in expression.
        np.random.binomial(1, 0.5),                         #  1: Is even.

        # Theorem choices.
        np.random.randint(3),                               #  2: Universal quantifier is [0] at beginning of th'm, [1] at end of th'm, [2] absent.
        np.random.randint(len(grammar.given)),              #  3: <given> choice.
        np.random.randint(len(grammar.any)),                #  4: <any> choice.
        np.random.randint(len(grammar.natural)),            #  5: <natural> choice.
        np.random.randint(len(grammar.expression)),         #  6: <expression> choice.
        np.random.randint(len(grammar.must_be)),            #  7: <must_be> choice.

        # Proof choices.
        np.random.binomial(1, 0.05),                        #  8: Is implicit. (i.e. doesn't mention every product is even.)
        np.random.randint(3),                               #  9: [0] Restate then prove, [1] prove then restate, [2] don't restate.
        # Implicit
        # TTB
        np.random.randint(len(grammar.imp_expression)),     # 10: <imp_expression> choice.
        np.random.randint(len(grammar.must_be)),            # 11: <must_be> choice.
        np.random.randint(len(grammar.since)),              # 12: <since> choice.
        np.random.randint(len(grammar.addition)),           # 13: <addition> choice.
        np.random.randint(len(grammar.even_natural)),       # 14: <even_natural> choice.
        np.random.randint(len(grammar.must_be)),            # 15: <must_be> choice.
        # BTT
        np.random.randint(len(grammar.observe)),            # 16: <observe> choice.
        np.random.randint(len(grammar.therefore)),          # 17: <therefore> choice.
        # No restatement
        np.random.randint(len(grammar.theorem)),            # 18: <theorem> choice.
        np.random.randint(len(grammar.follows)),            # 19: <follows> choice.
        # Explicit
        # TTB
        np.random.binomial(1, 0.5),                         # 20: Has logic flow. i.e. Since x, then y instead of just stating x.
        np.random.randint(len(grammar.known)),              # 21: <known> choice.
        np.random.randint(len(grammar.since)),              # 22: <since> choice.
        np.random.randint(len(grammar.therefore)),          # 23: <therefore> choice.

        np.random.binomial(1, 0.5),     # 24: List constants are even w/ product.
        np.random.randint(10),          # 25: Mention the constant.
        np.random.randint(10),          # 26: Mention even products are even.
    ])

    nums = np.array([np.random.randint(1, 1000) * 2 for i in range(seed[0] - 1)])
    const = np.random.randint(1, 1000) * 2 if seed[1] else np.random.randint(1, 1000) * 2 - 1
    ...
    if seed[2] == 0:
        latex += grammar.given[seed[3]]
        latex += grammar.any[seed[4]]

        if len(nums) < 2 and (grammar.any[seed[4]] == "" or grammar.any[seed[4]] == "all "):
            latex += "a "
        
        phrase = grammar.natural[seed[5]] if len(nums) > 1 else grammar.natural_singular[seed[5]]
        if phrase in [r"\in \mathbb{N} ", r"\in \mathbb{Z}^+ "]:
            latex += forall(vars, latex=True)
            latex += phrase + "$ "
        ...
        latex += grammar.expression[seed[6]]
        latex += expression(nums, vars, const, order, latex=True)
        latex += grammar.must_be[seed[7]]
        latex += ["odd . ", "even . "][seed[1]]
    elif seed[2] == 1:
        latex += expression(nums, vars, const, order, latex=True)
        latex += grammar.must_be[seed[7]]
        latex += ["odd ", "even "][seed[1]]
        latex += grammar.given[seed[3]].lower()
        latex += grammar.any[seed[4]]
    ...
    coq += ": nat , Nat . "
    coq += ["odd ", "even "][seed[1]]
    coq += "( " + expression(nums, vars, const, order) + ") "
    coq += "= true . "
    ...
    if seed[8]:
        if seed[9] == 0:
            ...
            if seed[26] == 0:
                latex += "The " + np.random.choice(grammar.product) + "of "
                latex += np.random.choice(grammar.even_natural_singular)
                latex += "with " + np.random.choice(grammar.any)
                latex += np.random.choice(grammar.natural[:-3])
                latex += "is even . "

            if seed[25] == 0:
                latex += np.random.choice(grammar.since).capitalize()
                latex += nat_to_token(const)
                latex += "is " + ["odd ", "even "][seed[1]] + ", "
                latex += grammar.imp_expression[seed[10]].lower()
                latex += expression(nums, vars, const, order, latex=True)
                latex += grammar.must_be[seed[11]]
                latex += ["odd ", "even "][seed[1]]
                latex += grammar.since[seed[12]]
                latex += grammar.addition[seed[13]] + "of "
                latex += grammar.even_natural[seed[14]]
                latex += "with an " + ["odd ", "even "][seed[1]] + "number "
                latex += grammar.must_be[seed[15]]
                latex += ["odd ", "even "][seed[1]] + ". "
            elif seed[25] == 1:
                latex += grammar.imp_expression[seed[10]]
                latex += expression(nums, vars, const, order, latex=True)
                latex += grammar.must_be[seed[11]]
                latex += ["odd ", "even "][seed[1]]
                latex += grammar.since[seed[12]]
            ...
            if seed[26] == 1:
                latex += np.random.choice(grammar.this_since).capitalize()
                latex += "the " + np.random.choice(grammar.product) + "of "
                latex += np.random.choice(grammar.even_natural_singular)
                latex += "with " + np.random.choice(grammar.any)
                latex += np.random.choice(grammar.natural[:-3])
            ...
            if seed[25] == 0:
                ...
                latex += grammar.must_be[seed[11]]
                latex += ["odd ", "even "][seed[1]] + ". "
                latex += grammar.therefore[seed[17]]
                latex += grammar.imp_expression[seed[10]].lower()
                latex += expression(nums, vars, const, order, latex=True)
                latex += grammar.must_be[seed[15]]
                latex += ["odd ", "even "][seed[1]] + ". "
            ...
            latex += grammar.theorem[seed[18]]
            latex += grammar.follows[seed[19]]
            latex += grammar.since[seed[12]]
            latex += grammar.addition[seed[13]] + "of "
            latex += grammar.even_natural[seed[14]]
            latex += "with an " + ["odd ", "even "][seed[1]] + "number "
        ...
        coq += "rewrite Nat . "
        coq += ["odd_add ", "even_add "][seed[1]]
        coq += ". "
        coq += ["rewrite <- Nat . negb_even . ", ""][seed[1]]
        coq += "repeat rewrite Nat . even_add . "
        coq += "repeat rewrite Nat . even_mul . "
        coq += "reflexivity . "
        coq += "Qed . "
        ...
        if seed[9] == 0:
            ...
            if seed[20]:
                latex += grammar.since[seed[22]].capitalize()

                now = np.random.binomial(1, 0.5)
                if seed[24] and now:
                    n = len(nums)
                    tmp_order = np.delete(order, np.where(order == n))
                    tmp_seed = np.random.randint(len(grammar.times))
                    for i in range(n - 1):
                        latex += "$ " + nat_to_token(nums[tmp_order[i]]) + "$ , "
                    if n > 1:
                        latex += "and "
                    latex += "$ " + nat_to_token(nums[tmp_order[n - 1]]) + "$ "
                    if n > 2:
                        latex += "are " + np.random.choice(grammar.even_natural)
                    else:
                        latex += "is " + np.random.choice(grammar.even_natural_singular)
                    latex += ", " + np.random.choice([
                        "it " + np.random.choice(grammar.follows) + "that ",
                        np.random.choice(grammar.known)
                    ])
                ...
                for i in range(n - 1):
                    latex += "$ " + nat_to_token(nums[tmp_order[i]])
                    latex += grammar.times[tmp_seed]
                    latex += var_to_token(vars[tmp_order[i]]) + "$ , "
                if n > 1:
                    latex += "and "
                latex += "$ " + nat_to_token(nums[tmp_order[n - 1]])
                latex += grammar.times[tmp_seed]
                latex += var_to_token(vars[tmp_order[n - 1]]) + "$ "
                
                if n > 1:
                    latex += grammar.evaluate[np.random.randint(len(grammar.evaluate))]
                    latex += grammar.even_natural[np.random.randint(len(grammar.even_natural))]
                else:
                    latex += grammar.evaluate_singular[np.random.randint(len(grammar.evaluate_singular))]
                    latex += grammar.even_natural_singular[np.random.randint(len(grammar.even_natural_singular))]

                if seed[24] and now:
                    latex += ". "
                    latex += np.random.choice(grammar.therefore)
                    latex += grammar.theorem[seed[18]].lower()
                    latex += grammar.follows[seed[19]] + ". "
                    latex += r"\end{proof} "
                else:
                    latex += ", " + grammar.theorem[seed[18]].lower()
                ...
                latex += grammar.known[seed[21]].capitalize()
                ...
                tmp_order = np.delete(order, np.where(order == n))
                while x < n:
                    y = x + np.random.randint(n - x)
                    fact_order = tmp_order[x:y + 1]

                    now = np.random.binomial(1, 0.5)
                    if seed[24] and now and np.random.binomial(1, 0.75):
                        latex += np.random.choice(grammar.since)
                        f = len(fact_order)
                        if f > 1:
                            tmp_seed = np.random.randint(len(grammar.times))
                            for i in range(f - 1):
                                latex += "$ " + nat_to_token(nums[fact_order[i]]) + "$ "
                                if f > 2:
                                    latex += ", "
                            latex += "and $ " + nat_to_token(nums[fact_order[f - 1]]) + "$ "
                            latex += "are " + np.random.choice(grammar.even_natural)
                            latex += ", " + np.random.choice(grammar.therefore).lower()
                            latex += np.random.choice([
                                "it " + np.random.choice(grammar.follows) + "that ",
                                np.random.choice(grammar.known)
                            ])
                        else:
                            latex += "$ " + nat_to_token(nums[fact_order[0]]) + "$ "
                            phrase = np.random.choice(grammar.even_natural_singular)
                            if phrase == "even ":
                                latex += "is " + phrase
                            else:
                                latex += "is an " + phrase
                    ...
                    if f > 1:
                        ...
                        for i in range(f - 1):
                            latex += "$ " + nat_to_token(nums[fact_order[i]])
                            latex += grammar.times[tmp_seed]
                            latex += var_to_token(vars[fact_order[i]]) + "$ "
                            if f > 2:
                                latex += ", "
                        latex += "and $ " + nat_to_token(nums[fact_order[f - 1]])
                        latex += grammar.times[tmp_seed]
                        latex += var_to_token(vars[fact_order[f - 1]]) + "$ "
                        latex += grammar.evaluate[np.random.randint(len(grammar.evaluate))]
                        latex += grammar.even_natural[np.random.randint(len(grammar.even_natural))] + ". "
                    else:
                        latex += "$ " + nat_to_token(nums[fact_order[0]])
                        latex += grammar.times[np.random.randint(len(grammar.times))]
                        latex += var_to_token(vars[fact_order[0]]) + "$ "
                        latex += grammar.evaluate_singular[np.random.randint(len(grammar.evaluate_singular))]
                        latex += grammar.even_natural_singular[np.random.randint(len(grammar.even_natural_singular))] + ". "

                    if seed[24] and not now and np.random.binomial(1, 0.75):
                        np.random.shuffle(fact_order)
                        latex += np.random.choice(grammar.this_since).capitalize()
                        f = len(fact_order)
                        if f > 1:
                    ...
                    x = y + 1
                    if x < n:
                        phrase = grammar.additionally[np.random.randint(len(grammar.additionally))].capitalize()
                        if phrase != "":
                            latex += phrase
                            latex += grammar.known[np.random.randint(len(grammar.known))]
                        else:
                            latex += grammar.known[np.random.randint(len(grammar.known))].capitalize()
            ...
            for idx in order:
                if idx != len(nums):
                    coq += "assert ( Nat . even ( "
                    coq += nat_to_token(nums[idx])
                    coq += "* "
                    coq += var_to_token(vars[idx])
                    coq += ") = true . ) as <genH:" + vars[idx] + "> . { "
                    coq += "rewrite Nat . even_mul . "
                    coq += "reflexivity . } "
        elif seed[9] == 1:
            np.random.shuffle(order)
            ...
            if seed[20]:
                ...
                now = np.random.binomial(1, 0.5)
                if seed[24] and now:
                    latex += np.random.choice(grammar.known)
                    n = len(nums)
                    tmp_order = np.delete(order, np.where(order == n))
                    tmp_seed = np.random.randint(len(grammar.times))
                    for i in range(n - 1):
            ...
            latex += grammar.therefore[seed[23]]
            if seed[25] == 0:
                latex += np.random.choice(grammar.since)
                latex += nat_to_token(const)
                latex += "is " + ["odd ", "even "][seed[1]] + ", "
            ...
            if seed[20]:
                ...
                if np.random.binomial(1, 0.5):
                    latex += grammar.since[np.random.randint(len(grammar.since))]
                    latex += grammar.addition[np.random.randint(len(grammar.addition))] + "of "
                    latex += grammar.even_natural[np.random.randint(len(grammar.even_natural))]
                    latex += "with an " + ["odd ", "even "][seed[1]] + "number "
                    latex += grammar.must_be[np.random.randint(len(grammar.must_be))]
                    latex += ["odd ", "even "][seed[1]]
                    if seed[25] == 1:
                        latex += np.random.choice(grammar.since)
                        latex += nat_to_token(const)
...
def powers_old(count):
    ...
    seed = [
        np.random.choice(num_options[args.set]),    # 0: Power for example.
        np.random.binomial(1, 0.5),                 # 1: Print definition.
        np.random.binomial(1, 0.5),                 # 2: Define term at beginning of definition.
        np.random.binomial(1, 0.5),                 # 3: Restate theorem.
        np.random.binomial(1, 0.5),                 # 4: Let expression.
        np.random.binomial(1, 0.5),                 # 5: Intermediate simplification.
        np.random.binomial(1, 0.5),                 # 6: Implicit restatement.
        np.random.binomial(1, 0.5),                 # 7: Use set notation in theorem statement.
        0,#np.random.binomial(1, 0.5),                 # 8: Verbose style powers.
        np.random.binomial(1, 0.5),                 # 9: Print variable in theorem statement when using a let or definition.
    ]
    ...
    if seed[8]:
        times = np.random.choice(grammar.times)
        power = var_to_token(vars[0])
        for i in range(seed[0] - 1):
            power += times + var_to_token(vars[0])

        coq_power = var_to_token(vars[0])
        for i in range(seed[0] - 1):
            coq_power += "* " + var_to_token(vars[0])
    else:
        power = var_to_token(vars[0]) + "^ " + nat_to_token(seed[0])
        coq_power = power
    
    if seed[1]:
        latex += r"\begin{definition} "
        if seed[2]:
            latex += "A <def:" + grammar.names[seed[0] - 2] + "> "
            latex += np.random.choice(grammar.said)
            
            term = np.random.choice(grammar.number)
            if term == "integer ":
                latex += np.random.choice(["some ", "any ", "an "]) + term
            else:
                latex += np.random.choice(["some ", "any ", "a "]) + term
            latex += "$ " + var_to_token(vars[2])
            latex += np.random.choice(grammar.in_nat)
            latex += "$ " + np.random.choice(grammar.such_that)
            latex += np.random.choice(grammar.exists)
        else:
            term = np.random.choice(["Some ", "Any "]) + np.random.choice(grammar.number)
            latex += "$ " + var_to_token(vars[2])
            latex += np.random.choice(grammar.in_nat)
            latex += "$ " + np.random.choice(grammar.said)
            latex += "a <def:" + grammar.names[seed[0] - 2] + "> "
        ...
        term = np.random.choice(grammar.number)
        if term == "integer ":
            latex += np.random.choice(["some ", "an "]) + term
        else:
            latex += np.random.choice(["some ", "a "]) + term
        latex += "$ " + var_to_token(vars[0])
        latex += np.random.choice(grammar.in_nat)
        latex += "$ " + np.random.choice(grammar.such_that)
        latex += "$ " + power
        ...
        coq += "Definition <def:" + grammar.names[seed[0] - 2] + "> ( " + var_to_token(vars[2]) + ": nat ) := "
        coq += "exists " + var_to_token(vars[0]) + ": nat , " + coq_power + "= " + var_to_token(vars[2]) + ". "
    ...
    if term != "":
        latex += "The "
    latex += term
    latex += nat_to_token(base ** seed[0])
    latex += np.random.choice(grammar.must_be)
    if seed[1]:
        latex += "a <def:" + grammar.names[seed[0] - 2] + "> "
    else:
        latex += "a power of " + nat_to_token(seed[0])
    
    if (not seed[1] and not seed[4]) or seed[9]:
        latex += np.random.choice(grammar.given)
        if seed[7]:
            latex += "$ " + var_to_token(vars[0])
            latex += r"\in " + np.random.choice([r"\mathbb{N} ", r"\mathbb{Z}^+ "]) + "$ "
    ...
    coq += "Theorem <genH:" + str(seed[0]) + "> : "
    if seed[1]:
        coq += "<def:" + grammar.names[seed[0] - 2] + "> ( " + nat_to_token(base ** seed[0]) + ") . "
    else:
        coq += "exists " + var_to_token(vars[0]) + " : nat , " + coq_power + "= " + nat_to_token(base ** seed[0]) + ". "

    # Proof.
    latex += r"\begin{proof} "
    if seed[4]:
        latex += np.random.choice(grammar.let)
        latex += "$ " + var_to_token(vars[0]) + "= " + nat_to_token(base) + "$ . "
        latex += np.random.choice(grammar.therefore)
        latex += "$ " + power
        if seed[5]:
        ...
        latex += np.random.choice(grammar.see)
        latex += "$ " + nat_to_token(base) + "^ " + nat_to_token(seed[0]) + "= "
        latex += nat_to_token(base ** seed[0]) + "$ . "
    ...
    if seed[3]:
        ...
        if seed[6]:
            latex += np.random.choice(grammar.theorem)
            latex += np.random.choice(grammar.follows)
            latex += ". "
        else:
            latex += nat_to_token(base ** seed[0])
            latex += np.random.choice(grammar.must_be)
    ...
    # Print.
    filename = args.path + "/" + args.set + "/powers_" + str(count) + "_tokenized.txt"
...
def composites_old(count):
    ...
    seed = [
        np.random.binomial(1, 0.5),                 # 0: Print definition.
        np.random.binomial(1, 0.5),                 # 1: Define term at beginning of definition.
        np.random.binomial(1, 0.5),                 # 2: Restate theorem.
        np.random.binomial(1, 0.5),                 # 3: Let expression.
        np.random.binomial(1, 0.5),                 # 4: Intermediate simplification.
        np.random.binomial(1, 0.5),                 # 5: Implicit restatement.
        np.random.binomial(1, 0.5),                 # 6: Use set notation in theorem statement.
        np.random.binomial(1, 0.5),                 # 7: Print variable in theorem statement when using a let or definition.
    ]

    x, y = np.random.randint(10000), np.random.randint(10000)
    vars = np.array(list(string.ascii_letters))
    np.random.shuffle(vars)
    vars = vars[:3]
    ...
    if seed[0]:
        ...
        latex += np.random.choice(["some ", "any "])
        latex += np.random.choice(grammar.numbers)
        latex += "$ " + var_to_token(vars[0])
        latex += np.random.choice(grammar.listing)
        latex += var_to_token(vars[1])
        latex += np.random.choice(grammar.in_nat)
        latex += "$ greater than or equal to <nat:2> " + np.random.choice(grammar.such_that)

        np.random.shuffle(order)
        latex += "$ " + var_to_token(vars[order[0]])
        latex += np.random.choice(grammar.times)
        latex += var_to_token(vars[order[1]])
        latex += np.random.choice(grammar.equal)
        latex += var_to_token(vars[2]) + "$ . "
        latex += r"\end{definition} "

        coq += "Definition <def:composite> ( " + var_to_token(vars[2]) + ": nat ) := "
        coq += "exists ( " + var_to_token(vars[0]) + var_to_token(vars[1]) + ": nat ) , ( "
        coq += var_to_token(vars[0]) + " >= <nat:2> ) /\ ( " + var_to_token(vars[1]) + " >= <nat:2> ) /\ ( "
        coq += var_to_token(vars[order[0]]) + "* " + var_to_token(vars[order[1]]) + "= " + var_to_token(vars[2]) + ") . "
    ...
    if term != "":
        latex += "The "
    latex += term
    latex += nat_to_token(x * y)
    latex += np.random.choice(grammar.must_be)

    coq += "Theorem <genH:" + str(x * y) + "> : "
    if seed[0]:
        coq += "<def:composite> ( " + nat_to_token(x * y) + ") . "

    np.random.shuffle(order)
    if seed[0]:
        latex += np.random.choice(["a <def:composite> number ", "<def:composite> "])
        if seed[7]:
            latex += np.random.choice(grammar.given)
            if seed[6]:
                latex += "$ " + var_to_token(vars[order[0]])
                ...
                latex += np.random.choice(["some ", "any "]) + np.random.choice(grammar.numbers)
                latex += "$ " + var_to_token(vars[order[0]])
                latex += np.random.choice(grammar.listing)
                latex += var_to_token(vars[order[1]]) + "$ "
    else:
        latex += "a product of " + defs[seed[0]]
        latex += np.random.choice(grammar.numbers)
        if not seed[3] or seed[7]:
            latex += "$ " + var_to_token(vars[order[0]])
            latex += np.random.choice(grammar.listing)
            latex += var_to_token(vars[order[1]])
            latex += r"\in " + np.random.choice([r"\mathbb{N} ", r"\mathbb{Z}^+ "]) + "$ "
            ...
            if not seed[3]:
                coq += "exists ( " + var_to_token(vars[order[0]]) + var_to_token(vars[order[1]]) + " : nat , ( "
                coq += var_to_token(vars[order[0]]) + ">= <nat:2> ) /\ ( "
                coq += var_to_token(vars[order[1]]) + ">= <nat:2> ) /\ ( "
                coq += var_to_token(vars[order[0]]) + "* " + var_to_token(vars[order[1]]) + "= " + nat_to_token(x * y) + ") . "
        latex += "greater than or equal to <nat:2> "
    latex += r". \end{theorem} "
    ...
    if seed[3]:
        latex += np.random.choice(grammar.let)
        latex += "$ " + var_to_token(vars[order[0]]) + "= " + nat_to_token(x) + "$ and "
        latex += "$ " + var_to_token(vars[order[1]]) + "= " + nat_to_token(y) + "$ . "
        ...
        times = np.random.choice(grammar.times)
        latex += np.random.choice(grammar.therefore)
        latex += "$ " + var_to_token(vars[new_order[0]])
        latex += times
        latex += var_to_token(vars[new_order[1]])
        if seed[4] and new_order[0] == order[0]:
            latex += "= " + nat_to_token(x) + times + nat_to_token(y)
        elif seed[4]:
            latex += "= " + nat_to_token(y) + times + nat_to_token(x)
        if not seed[4]:
            latex += np.random.choice(grammar.equal)
        latex += nat_to_token(x * y) + "$ . "
    else:
        latex += np.random.choice(grammar.see)
        latex += "$ " + nat_to_token(x) + np.random.choice(grammar.times) + nat_to_token(y) + "= "
        latex += nat_to_token(x * y) + "$ . "
    ...
    if seed[2]:
        ...
        if seed[5]:
            ...
            latex += np.random.choice(grammar.must_be)
            if seed[0]:
                latex += "a <def:composite> . "
            else:
                latex += "as in the theorem . "
    latex += r"\end{proof} \end{document} "

    coq += "Proof . "
    coq += "exists " + nat_to_token(x) + ". "
    coq += "exists " + nat_to_token(y) + ". "
    coq += "split . "
    coq += "auto . "
    coq += "split . "
    coq += "auto . "
    ...
    # Print.
    filename = args.path + "/" + args.set + "/composites_" + str(count) + "_tokenized.txt"
